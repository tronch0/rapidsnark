...
function renderCircom() {
  pushd "$CIRCUIT_DIR"
  echo sed -i '' "s/Main([0-9]*)/Main($INPUT_SIZE)/" inclusion.circom
  sed -i '' "s/Main([0-9]*)/Main($INPUT_SIZE)/" inclusion.circom
  popd
}

function compile() {
  pushd "$CIRCUIT_DIR"
  echo circom inclusion.circom --r1cs --sym --wasm -o compiled
  circom inclusion.circom --r1cs --sym --wasm -o compiled
  popd
}
...
function setup() {
  ...
  prove_key_size=$(ls -lh "$CIRCUIT_DIR"/compiled/inclusion_0001.zkey | awk '{print $5}')
...
function generateWtns() {
  pushd "$CIRCUIT_DIR"
  echo node compiled/inclusion_js/generate_witness.js compiled/inclusion_js/inclusion.wasm ../input/input_${INPUT_SIZE}.json compiled/witness.wtns
  "${TIME[@]}" node compiled/inclusion_js/generate_witness.js compiled/inclusion_js/inclusion.wasm ../input/input_${INPUT_SIZE}.json compiled/witness.wtns
  popd
}
...
#  pushd "$CIRCUIT_DIR"
#  avg_time 10 snarkjs groth16 prove inclusion_0001.zkey witness.wtns proof.json public.json
...
function rapidProve() {
  pushd "$CIRCUIT_DIR"
  avg_time 10 "$RAPID_SNARK_PROVER" compiled/inclusion_0001.zkey compiled/witness.wtns compiled/proof.json compiled/public.json
...
function verify() {
  pushd "$CIRCUIT_DIR"
  avg_time 10 snarkjs groth16 verify compiled/verification_key.json compiled/public.json compiled/proof.json
  popd
}
